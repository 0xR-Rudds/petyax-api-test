...
local function SaveConfig(name)
    local config = {
        ...
        Colors = {
            MainGradientTop = MainGradientTopColor,
            MainGradientBottom = MainGradientBottomColor,
            NeonBorder = NeonBorderColor,
            AccentStroke = AccentStrokeColor,
            TextColor = TextColorColor,
        },
    }
    ...
    if success then
        writefile('PetyaX_' .. name .. '.json', result)
        CurrentConfig = name
        return true
    else
        return false
...
local function LoadConfig(name)
    local success, content = pcall(function()
        return readfile('PetyaX_' .. name .. '.json')
    end)
        ...
        if success2 and config then
            ...
            if config.Aimbot then
                for key, value in pairs(config.Aimbot) do
                    if getgenv().Aimbot[key] ~= nil then
            ...
            if config.Crosshair then
                for key, value in pairs(config.Crosshair) do
                    if getgenv().Crosshair[key] ~= nil then
            ...
            if config.Colors then
                MainGradientTopColor = config.Colors.MainGradientTop
                    or MainGradientTopColor
                MainGradientBottomColor = config.Colors.MainGradientBottom
                    or MainGradientBottomColor
                NeonBorderColor = config.Colors.NeonBorder or NeonBorderColor
                AccentStrokeColor = config.Colors.AccentStroke
                    or AccentStrokeColor
                TextColorColor = config.Colors.TextColor or TextColorColor
                ApplyColors()
            end
...
local function RefreshConfigList()
    ...
    if success then
        for _, file in pairs(files) do
            if string.find(file, 'PetyaX_') and string.find(file, '.json') then
                local configName = string.gsub(file, 'PetyaX_', '')
...
function UpdateESP()
    for player, drawings in pairs(ESPDrawings) do
        ...
        local espColor = getgenv().ESP.Rainbow and GetRainbowColor()
            or getgenv().ESP.BoxColor
        ...
        if getgenv().ESP.Tracers then
            ...
            drawings.Tracer.Color = getgenv().ESP.Rainbow and GetRainbowColor()
                or getgenv().ESP.TracerColor
            drawings.Tracer.Thickness = getgenv().ESP.TracerThickness
        ...
        if getgenv().ESP.Names then
            ...
            drawings.Name.Color = getgenv().ESP.Rainbow and GetRainbowColor()
                or getgenv().ESP.NameColor
        ...
        if getgenv().ESP.Distance then
            ...
            drawings.Distance.Position =
                Vector2.new((minX + maxX) / 2, maxY + 5)
        ...
        if getgenv().ESP.HealthBar then
            ...
            drawings.HealthBar.Position =
                Vector2.new(barX, barY + (barHeight - healthHeight))
...
function FindTarget()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        ...
        if getgenv().Aimbot.TeamCheck and player.Team == LocalPlayer.Team then
            continue
        end
        ...
        if getgenv().Aimbot.VisibleCheck then
            local ray = Ray.new(
                ...
                (head.Position - Camera.CFrame.Position).Unit * distance
            )
...
function MoveCrosshairToTarget(target)
    ...
    if getgenv().Aimbot.Prediction > 0 then
        ...
        if humanoid and humanoid.MoveDirection.Magnitude > 0 then
            ...
            local velocity = moveDirection
                * humanoid.WalkSpeed
                * getgenv().Aimbot.Prediction
            ...
            if distance > 50 then
                local gravityCompensation = Vector3.new(
                    0,
                    (distance * 0.0005) * getgenv().Aimbot.Prediction,
                    0
                )
    ...
    mousemoverel(math.floor(smoothedDelta.X), math.floor(smoothedDelta.Y))
end
...
function UpdateCrosshair()
    ...
    local crosshairColor = getgenv().Crosshair.RainbowCrosshair
            and GetRainbowColor()
        or getgenv().Crosshair.Color
...
CloseButton.MouseEnter:Connect(function()
    if CloseStroke then
        TweenService:Create(
            CloseStroke,
            TweenInfo.new(0.1),
            { Transparency = 0, Color = NeonBorderColor }
        ):Play()
    end
end)
...
CloseButton.MouseLeave:Connect(function()
    if CloseStroke then
        TweenService:Create(
            CloseStroke,
            TweenInfo.new(0.1),
            { Transparency = 0.4, Color = AccentStrokeColor }
        ):Play()
    end
end)
...
-- =============================================================================
-- UPDATED TAB SYSTEM WITH BOXED SECTIONS STYLE
-- =============================================================================
...
local function createTab(name, index)
    ...
    TabButton.BackgroundColor3 = SectionBoxColor
    ...
    TabButtonStroke.Color = AccentStrokeColor
    TabButtonStroke.Thickness = 1
    TabButtonStroke.Transparency = name == 'ESP' and 0.3 or 0.6
    ...
    TabTextButton.MouseEnter:Connect(function()
        ...
        TweenService
            :Create(TabButtonStroke, TweenInfo.new(0.1), { Transparency = 0.3 })
            :Play()
    end)

    TabTextButton.MouseLeave:Connect(function()
    ...
    TabTextButton.MouseButton1Click:Connect(function()
        ...
        for _, button in pairs(UIReferences.tabButtons) do
            if button and button:FindFirstChildOfClass('UIStroke') then
                TweenService
                    :Create(
...
-- =============================================================================
-- NEW: SECTION BOX CREATION FUNCTION
-- =============================================================================
...
-- =============================================================================
-- ESP TAB CONTENT WITH BOXED SECTIONS
-- =============================================================================
...
ESPLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
    ESPScroll.CanvasSize =
        UDim2.new(0, 0, 0, ESPLayout.AbsoluteContentSize.Y + 10)
end)
...
local function CreateESPCheckbox(
    parent,
    labelText,
    configKey,
    defaultValue,
...
local BoxesCheckFrame =
    CreateESPCheckbox(ESPFeaturesSection, 'Boxes', 'Boxes', true, 1)
local TracersCheckFrame =
    CreateESPCheckbox(ESPFeaturesSection, 'Tracers', 'Tracers', true, 2)
local NamesCheckFrame =
    CreateESPCheckbox(ESPFeaturesSection, 'Names', 'Names', true, 3)
local DistanceCheckFrame =
    CreateESPCheckbox(ESPFeaturesSection, 'Distance', 'Distance', true, 4)
local HealthBarCheckFrame =
    CreateESPCheckbox(ESPFeaturesSection, 'Health Bar', 'HealthBar', true, 5)
...
local TeamESPCheckFrame =
    CreateESPCheckbox(ESPSettingsSection, 'Team Check', 'TeamCheck', true, 1)
local RainbowESPCheckFrame =
    CreateESPCheckbox(ESPSettingsSection, 'Rainbow ESP', 'Rainbow', true, 2)
...
local ESPConfigSection =
    CreateSectionBox(ESPScroll, 'ESP CONFIGURATION', 4, 180)
...
-- ESP Slider Function
local function CreateESPSlider(
    parent,
    labelText,
    configKey,
    minValue,
    ...
    SliderFill.Size =
        UDim2.new((defaultValue - minValue) / (maxValue - minValue), 0, 1, 0)
    ...
    Slider.MouseButton1Down:Connect(function()
        ...
        connection = RunService.Heartbeat:Connect(function()
            ...
            local relativeX =
                math.clamp((mousePos.X - sliderPos.X) / sliderSize.X, 0, 1)
...
local function CreateESPThicknessSlider(
    parent,
    labelText,
    configKey,
    minValue,
...
-- Create ESP Sliders
local ESPDistanceSlider = CreateESPSlider(
    ESPConfigSection,
    'Max Distance',
    'MaxDistance',
    50,
...
local BoxThicknessSlider = CreateESPThicknessSlider(
    ESPConfigSection,
    'Box Thickness',
    'BoxThickness',
    1,
...
local TracerThicknessSlider = CreateESPThicknessSlider(
    ESPConfigSection,
    'Tracer Thickness',
    'TracerThickness',
    1,
...
-- =============================================================================
-- AIMBOT TAB CONTENT WITH BOXED SECTIONS
-- =============================================================================
...
AimbotLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
    AimbotScroll.CanvasSize =
        UDim2.new(0, 0, 0, AimbotLayout.AbsoluteContentSize.Y + 10)
end)
...
local AimbotSettingsSection =
    CreateSectionBox(AimbotScroll, 'AIMBOT SETTINGS', 2, 180)
...
local function CreateAimbotCheckbox(
    parent,
    labelText,
    configKey,
    defaultValue,
...
local TeamCheckFrame = CreateAimbotCheckbox(
    AimbotSettingsSection,
    'Team Check',
    'TeamCheck',
    true,
    1
)
local VisibleCheckFrame = CreateAimbotCheckbox(
    AimbotSettingsSection,
    'Visible Check',
    'VisibleCheck',
    true,
...
local RainbowFOVFrame = CreateAimbotCheckbox(
    AimbotSettingsSection,
    'Rainbow FOV',
    'RainbowFOV',
    false,
...
local ToggleModeFrame = CreateAimbotCheckbox(
    AimbotSettingsSection,
    'Toggle Mode',
    'Toggle',
    false,
...
local AimbotConfigSection =
    CreateSectionBox(AimbotScroll, 'AIMBOT CONFIGURATION', 3, 180)
...
local function CreateAimbotSlider(
    parent,
    labelText,
    configKey,
    minValue,
    ...
    local function updateValue(value)
        ...
        if decimals == 0 then
            ...
            scaledValue = math.floor(scaledValue * (10 ^ decimals))
                / (10 ^ decimals)
        end
        ...
        if decimals == 0 then
            ...
            Label.Text = labelText
                .. ': '
                .. string.format('%.' .. decimals .. 'f', scaledValue)
        end
...
-- UPDATED: Create Aimbot Sliders with new ranges
local FOVSlider =
    CreateAimbotSlider(AimbotConfigSection, 'Aim FOV', 'FOV', 10, 600, 80, 0, 1) -- Max FOV 600
local SmoothSlider = CreateAimbotSlider(
    AimbotConfigSection,
    'Smoothing',
    'Smoothing',
    0.00,
...
local PredictionSlider = CreateAimbotSlider(
    AimbotConfigSection,
    'Prediction',
    'Prediction',
    0,
...
-- Target Part Section
local TargetPartSection =
    CreateSectionBox(AimbotScroll, 'TARGET SETTINGS', 4, 80)
...
-- =============================================================================
-- CROSSHAIR TAB CONTENT WITH BOXED SECTIONS
-- =============================================================================
...
CrosshairLayout:GetPropertyChangedSignal('AbsoluteContentSize')
    :Connect(function()
        CrosshairScroll.CanvasSize =
            UDim2.new(0, 0, 0, CrosshairLayout.AbsoluteContentSize.Y + 10)
    end)

-- Crosshair Main Section
local CrosshairMainSection =
    CreateSectionBox(CrosshairScroll, 'CROSSHAIR MAIN', 1, 80)
...
local CrosshairFeaturesSection =
    CreateSectionBox(CrosshairScroll, 'CROSSHAIR FEATURES', 2, 80)
...
local function CreateCrosshairCheckbox(
    parent,
    labelText,
    configKey,
    defaultValue,
...
local CenterDotFrame = CreateCrosshairCheckbox(
    CrosshairFeaturesSection,
    'Center Dot',
    'CenterDot',
    true,
    1
)
local RainbowCrosshairFrame = CreateCrosshairCheckbox(
    CrosshairFeaturesSection,
    'Rainbow Crosshair',
    'RainbowCrosshair',
    true,
...
local CrosshairConfigSection =
    CreateSectionBox(CrosshairScroll, 'CROSSHAIR CONFIGURATION', 3, 200)
...
local function CreateCrosshairSlider(
    parent,
    labelText,
    configKey,
    minValue,
...
local CrosshairSizeSlider = CreateCrosshairSlider(
    CrosshairConfigSection,
    'Crosshair Size',
    'Size',
    8,
    24,
...
local CrosshairThicknessSlider = CreateCrosshairSlider(
    CrosshairConfigSection,
    'Crosshair Thickness',
    'Thickness',
    1,
...
local CrosshairGapSlider = CreateCrosshairSlider(
    CrosshairConfigSection,
    'Gap Size',
    'GapSize',
    2,
...
local CrosshairTransparencySlider = CreateCrosshairSlider(
    CrosshairConfigSection,
    'Transparency',
    'Transparency',
    0,
...
local CrosshairColorSection =
    CreateSectionBox(CrosshairScroll, 'CROSSHAIR COLOR', 4, 80)
...
-- =============================================================================
-- CONFIG TAB CONTENT WITH BOXED SECTIONS
-- =============================================================================
...
ConfigLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
    ConfigScroll.CanvasSize =
        UDim2.new(0, 0, 0, ConfigLayout.AbsoluteContentSize.Y + 10)
end)

-- Config Management Section
local ConfigManagementSection =
    CreateSectionBox(ConfigScroll, 'CONFIG MANAGEMENT', 1, 200)
...
ConfigButtonsFrame.Size = UDim2.new(1, 0, 0, 120)
...
local ConfigButtonsLayout = Instance.new('UIGridLayout')
ConfigButtonsLayout.CellSize = UDim2.new(1, 0, 0, 30)
ConfigButtonsLayout.CellPadding = UDim2.new(0, 0, 0, 5)
ConfigButtonsLayout.SortOrder = Enum.SortOrder.LayoutOrder
ConfigButtonsLayout.Parent = ConfigButtonsFrame

-- Create Config Button
local CreateConfigButton = Instance.new('TextButton')
CreateConfigButton.Size = UDim2.new(1, 0, 0, 30)
CreateConfigButton.BackgroundColor3 = NeonBorderColor
CreateConfigButton.TextColor3 = MainGradientTopColor
CreateConfigButton.Text = 'Create Config'
CreateConfigButton.TextSize = 12
CreateConfigButton.Font = Enum.Font.Code
CreateConfigButton.Parent = ConfigButtonsFrame

local CreateConfigStroke = Instance.new('UIStroke')
CreateConfigStroke.Color = AccentStrokeColor
CreateConfigStroke.Thickness = 1
CreateConfigStroke.Parent = CreateConfigButton

local CreateConfigCorner = Instance.new('UICorner')
CreateConfigCorner.CornerRadius = UDim.new(0, 4)
CreateConfigCorner.Parent = CreateConfigButton
...
local LoadConfigStroke = Instance.new('UIStroke')
LoadConfigStroke.Color = AccentStrokeColor
LoadConfigStroke.Thickness = 1
LoadConfigStroke.Parent = LoadConfigButton
...
OverwriteConfigButton.BackgroundColor3 = NeonBorderColor
OverwriteConfigButton.TextColor3 = MainGradientTopColor
OverwriteConfigButton.Text = 'Overwrite Config'
OverwriteConfigButton.TextSize = 12
OverwriteConfigButton.Font = Enum.Font.Code
OverwriteConfigButton.Parent = ConfigButtonsFrame

local OverwriteConfigStroke = Instance.new('UIStroke')
OverwriteConfigStroke.Color = AccentStrokeColor
OverwriteConfigStroke.Thickness = 1
OverwriteConfigStroke.Parent = OverwriteConfigButton

local OverwriteConfigCorner = Instance.new('UICorner')
OverwriteConfigCorner.CornerRadius = UDim.new(0, 4)
OverwriteConfigCorner.Parent = OverwriteConfigButton

-- Delete Config Button
local DeleteConfigButton = Instance.new('TextButton')
DeleteConfigButton.Size = UDim2.new(1, 0, 0, 30)
DeleteConfigButton.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
DeleteConfigButton.TextColor3 = TextColorColor
DeleteConfigButton.Text = 'Delete Config'
DeleteConfigButton.TextSize = 12
DeleteConfigButton.Font = Enum.Font.Code
DeleteConfigButton.Parent = ConfigButtonsFrame

local DeleteConfigStroke = Instance.new('UIStroke')
DeleteConfigStroke.Color = AccentStrokeColor
DeleteConfigStroke.Thickness = 1
DeleteConfigStroke.Parent = DeleteConfigButton

local DeleteConfigCorner = Instance.new('UICorner')
DeleteConfigCorner.CornerRadius = UDim.new(0, 4)
DeleteConfigCorner.Parent = DeleteConfigButton

-- Config List Section
local ConfigListSection =
    CreateSectionBox(ConfigScroll, 'SAVED CONFIGS', 2, 200)
...
local ConfigListInnerLayout = Instance.new('UIListLayout')
ConfigListInnerLayout.SortOrder = Enum.SortOrder.LayoutOrder
ConfigListInnerLayout.Padding = UDim.new(0, 2)
ConfigListInnerLayout.Parent = ConfigListFrame

-- Current Config Display
local CurrentConfigSection =
    CreateSectionBox(ConfigScroll, 'CURRENT CONFIG', 3, 80)
local CurrentConfigLayout = Instance.new('UIListLayout')
CurrentConfigLayout.SortOrder = Enum.SortOrder.LayoutOrder
CurrentConfigLayout.Padding = UDim.new(0, 5)
CurrentConfigLayout.Parent = CurrentConfigSection

local CurrentConfigLabel = Instance.new('TextLabel')
CurrentConfigLabel.Size = UDim2.new(1, 0, 0, 20)
CurrentConfigLabel.BackgroundTransparency = 1
CurrentConfigLabel.Text = 'Current Config: None'
CurrentConfigLabel.TextColor3 = TextColorColor
CurrentConfigLabel.TextSize = 14
CurrentConfigLabel.Font = Enum.Font.Code
CurrentConfigLabel.TextXAlignment = Enum.TextXAlignment.Left
CurrentConfigLabel.LayoutOrder = 1
...
local function UpdateConfigList()
    ...
    for _, configName in pairs(Configs) do
        ...
        ConfigItem.BackgroundColor3 = MainGradientTopColor
        ConfigItem.TextColor3 = TextColorColor
        ConfigItem.Text = configName
        ConfigItem.TextSize = 12
        ConfigItem.Font = Enum.Font.Code
        ConfigItem.Parent = ConfigListFrame

        local ConfigItemStroke = Instance.new('UIStroke')
        ConfigItemStroke.Color = AccentStrokeColor
        ConfigItemStroke.Thickness = 1
        ConfigItemStroke.Parent = ConfigItem

        local ConfigItemCorner = Instance.new('UICorner')
        ConfigItemCorner.CornerRadius = UDim.new(0, 4)
        ...
        ConfigItem.MouseLeave:Connect(function()
            TweenService:Create(
                ConfigItem,
                TweenInfo.new(0.2),
                { BackgroundColor3 = MainGradientTopColor }
            ):Play()
        end)
    end

    ConfigListFrame.CanvasSize =
        UDim2.new(0, 0, 0, ConfigListInnerLayout.AbsoluteContentSize.Y)
end
...
CreateConfigButton.MouseButton1Click:Connect(function()
    ...
    if configName and configName ~= '' then
        if SaveConfig(configName) then
            CurrentConfigLabel.Text = 'Current Config: ' .. configName
            UpdateConfigList()
        end
    end
end)
...
LoadConfigButton.MouseButton1Click:Connect(function()
    ...
    if configName and configName ~= '' then
        if LoadConfig(configName) then
            ...
            -- This would need to be implemented for each UI element
        end
    end
end)
...
-- =============================================================================
-- SETTINGS TAB CONTENT WITH BOXED SECTIONS
-- =============================================================================
...
SettingsLayout:GetPropertyChangedSignal('AbsoluteContentSize')
    :Connect(function()
        SettingsScroll.CanvasSize =
            UDim2.new(0, 0, 0, SettingsLayout.AbsoluteContentSize.Y + 10)
    end)
...
local function ApplyColors()
    ...
    for _, button in pairs(UIReferences.tabButtons) do
        if button then
            button.BackgroundColor3 = SectionBoxColor
        end
    end
...
local KeybindSection =
    CreateSectionBox(SettingsScroll, 'KEYBIND SETTINGS', 2, 120)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == ToggleKey then
        ...
        VisibilityLabel.Text = 'Menu: '
            .. (GuiVisible and 'Visible' or 'Hidden')
...
print('💾 Config System: JSON-based save/load system')
